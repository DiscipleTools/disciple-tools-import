...
/**
 * Class Disciple_Tools_Import_Tab_Group
 */
class Disciple_Tools_Import_Tab_Group
{
    ...
    public function mapping_process( $filepath, $file_assigned_to = '' ){
        ...
        $group_headers_info = self::get_group_header_info();
        $group_headers_info_keys = array_keys( $group_headers_info );
        ...
        if ( $incl_headers == "yes" && isset( $data_rows[0] ) ) {
            ...
            $csv_headers = $group_headers_info_keys;
        }

        $temp_groups_data = $data_rows;
    ...
    public function preview( $mapping_data = [], $value_mapperi_data = [], $value_mapper_data = [] ) {
        ...
        $import_settings = get_transient( "disciple_tools_import_settings" );
        $groups = self::process_data( $import_settings, $mapping_data, $value_mapperi_data, $value_mapper_data );
        $import_settings["groups"] = $groups;
        set_transient( "disciple_tools_import_settings", $import_settings, 3600 * 24 );
        ?>
            ...
            <tr>
                <td>
                    ...
                    <script type="text/javascript">
                        ...
                        function doEach( item, done ) {
                            ...
                            jQuery.ajax({
                                ...
                                dataType: "json",
                                url: "<?php echo esc_url_raw( rest_url() ); ?>" + `dt-posts/v2/groups?silent=true`,
                    ...
                    <script type="text/javascript">
                        ...
                        let queue = <?php echo wp_json_encode( $js_groups ); ?>;
                        process(queue, 5, doEach, doDone);
                    </script>

                    <?php
                    $num = count( $groups );
                    echo esc_html( sprintf( __( "Creating %s Groups DO NOT LEAVE THE PAGE the \"All Done\" message appears", 'disciple_tools' ), $num ) );
                    ?>
    ...
    public function get_group_header_info(){

        $data = [];
        $channels = DT_Posts::get_post_settings( "groups" )["channels"];
    ...
    public static function get_dropdown_list_html( $field, $id = 'selector', $html_options = [] ) {
        ...
        $channels = DT_Posts::get_post_settings( "groups" )['channels'];
        $data = DT_Posts::get_post_field_settings( "groups" );
        ?>
        ...
        >
            ...
            <optgroup label="Standard Fields">
                ...
                <option value="title" <?php selected( $field == 'title' ) ?> >Group Name</option>
            ...
            <optgroup label="Other Fields">
                ...
                foreach ( $list_data as $key => $label ) {
                    ?>
                    <option value="<?php echo esc_html( $key ); ?>" <?php selected( $field != null && $field == $key || $group_field != null && $group_field == $key ) ?>><?php echo esc_html( $label ); ?></option>
                <?php } ?>
            </optgroup>
        </select>
        <?php
    ...
    public static function process_data( $import_settings, $mapping_data = [], $value_mapperi_data = [], $value_mapper_data = [] ){
        ...
        $data_rows = $import_settings["data"]["temp_groups_data"] ?? [];
       ...
       // $source = $import_settings["source"] ?? "";
        ...
        $cfs = DT_Posts::get_post_field_settings( "groups" );
     ...
     * @param array $groups
     */
    public static function display_data( $groups ) {
        ...
        $error_summary = [];
        if ( isset( $groups[0][0] ) ) {
            $headings = array_keys( $groups[0][0] );
        ...
        <table class="data-table">
            ...
            <tr>
                ...
                <th data-col-id=1>
                    <span class="cflabel">
                    <?php esc_html_e( 'Group Name', 'disciple_tools' ); ?>
                    </span>
                </th>
        ...
        <?php
        $total_data_rows = count( (array) $groups );
    ...
    public static function get_all_default_values() {
        $data = array();
        $data['groups'] = DT_Posts::get_post_settings( "groups" )["channels"];
        $data['fields'] = DT_Posts::get_post_field_settings( "groups" );
        return $data;
    }
