...
class DT_Import_Export_Tab_Contact {
    ...
    public function mapping( $filepath, $file_source = 'web', $file_assigned_to = '' ) {
        ...
        $uploade_file_headers = $data['uploadeFileHeaders'];
        $my_optional_fields = $data['myOptionalFields'];
...
<td>
    ...
    public function mapping_process( $filepath, $file_source = 'web', $file_assigned_to = '' ) {
        ...
        $delimeter = ','; $incl_headers = 'yes'; $multi_separator = ';';
        ...
        $uploade_file_headers = [];

        $my_optional_fields = self::get_all_default_values();
        ...
        if ( $incl_headers == "yes" && isset( $data_rows[0] ) ) {
            ...
            $uploade_file_headers = $data_rows[0];
            unset($data_rows[0]);
...
//echo '<hr/>OPT_FLDS:<br/><pre>'; print_r($my_optional_fields); echo '</pre>';
//echo '<hr/>HEADER:<br/><pre>'; print_r($con_headers_info); echo '</pre>';
//echo '<hr/>KEYS:<br/><pre>'; print_r($con_headers_infoKeys); echo '</pre>';
...
/******************************************************************************/
        ...
        foreach ( $csv_headers as $ci => $ch ) {
            ...
            if ( $my_mapper_column != null && strlen( $my_mapper_column )>0 ) { $csv_headers[$ci] = $my_mapper_column; }
        }
...
/******************************************************************************/
        ...
        foreach ( $unique as $ci => $list ) {
            ...
            $ch = $csv_headers[$ci];
            //if ( isset($my_optional_fields['fields'][$ch]['type'])
            //         &&$my_optional_fields['fields'][$ch]['type'] == 'multi_select' ) {
                //$multi_separator = ';';
        ...
        return [
            ...
            'uploadeFileHeaders' => $uploade_file_headers,
            'myOptionalFields' => $my_optional_fields,
    ...
    public function preview_process( $csv_data = [], $csv_headers = [], $value_mapper_idata = [], $value_mapper_data = [], $file_assigned_to = '', $file_source = '', $delimeter = ',' ) {
        return self::process_data( $csv_data, $csv_headers, $value_mapper_idata, $value_mapper_data, $file_assigned_to, $file_source, $delimeter);
    }

    public function preview( $filepath, $csv_data, $csv_headers, $delimeter = ',', $multiseperator = ';', $file_source = 'web', $file_assigned_to = '', $mapping_data, $value_mapper_idata, $value_mapper_data ) {

        $con_headers_info = self::get_contact_header_info();
        foreach ( (array) $mapping_data as $my_map_index => $my_map_data ) {
            if ( $my_map_data == 'IGNORE' || $my_map_data == 'NONE' ) {
                unset( $mapping_data[$my_map_index] );
            }
        }
...
////////////////////////////////////////////////////////////////////////////////
    ...
    public static function process_data(
        ...
        foreach ( $data_rows as $row_id => $row_data ) {
                ...
                if ( isset( $csv_headers[$col_id] ) ) {
                    ...
                    if ( isset( $mids[$ch]['columIds'] ) ) {
                        foreach ( $mids[$ch]['columIds'] as $xcolid ) {
                            if ( $col_id == $xcolid ) {
                                $data_rows[$row_id][ $mids[$ch]['primaryCol'] ] .= $multi_separator.$col_data;
                                unset( $data_rows[$row_id][$xcolid] );
                            }
                        }
                    }
                }
...
////////////////////////////////////////////////////////////////////////////////
        ...
        foreach ( $error_summary as $ch => $err ) {
            ...
            } else if ( $column_type == 'date' ) { $error_html .= ' needs to be in format yyyy-mm-dd';
            } else { $error_html .= ' needs to contain valid format';
            }
...
////////////////////////////////////////////////////////////////////////////////
                ...
                //} else if ( $ch == 'cf_notes'||$ch == 'cf_dob'||$ch == 'cf_join_date' ) {
                    if ( isset( $person_data[$ch][0] ) ) {  $html .= esc_html( $person_data[$ch][0] );
                    } else { $html .= 'None'; }
////////////////////////////////////////////////////////////////////////////////
                } else {
